...
function [] = train_patch_with_tumor_512_basePatch(name1)

    global filename
    global filename1
    global viablemask
    global k
    global train_label_matrix
...
%     global label_matrix
    ...
    if ((tissue_region / (512*512) > 0.25) && (count_bg /(512 * 512) < 0.7) && (row == 512) && (col == 512))    %check the ratio of total no of pixels in [512 * 512] image , to remove the background patches.     
        ...
        % First extract patch of size 256x256
        xmin_L3 = (x_cen - 128);
        ymin_L3 = (y_cen - 128);
        width_L3 = 255;
        height_L3 = 255;
        ...
        %figure,imshow(J2_5x);
        output_filename = strcat('/raid/data/home/matlab/Mousumi/patch_dataset/MultiResolution/5x/img/Train/',sprintf('%s_%d_%d.tiff', filename, block_struct.location(1),block_struct.location(2)));
        output_filename1 = strcat('/raid/data/home/matlab/Mousumi/patch_dataset/MultiResolution/5x/gt/Train/',sprintf('%s_%d_%d.tiff', strcat(filename,'_viable'), block_struct.location(1),block_struct.location(2)));
        ...
        output_filename = strcat('/raid/data/home/matlab/Mousumi/patch_dataset/MultiResolution/10x/img/Train/',sprintf('%s_%d_%d.tiff', filename, block_struct.location(1),block_struct.location(2)));
        output_filename1 = strcat('/raid/data/home/matlab/Mousumi/patch_dataset/MultiResolution/10x/gt/Train/',sprintf('%s_%d_%d.tiff', strcat(filename,'_viable'), block_struct.location(1),block_struct.location(2)));
        ...
        output_filename = strcat('/raid/data/home/matlab/Mousumi/patch_dataset/MultiResolution/20x/img/Train/',sprintf('%s_%d_%d.tiff', filename, block_struct.location(1),block_struct.location(2)));
        output_filename1 = strcat('/raid/data/home/matlab/Mousumi/patch_dataset/MultiResolution/20x/gt/Train/',sprintf('%s_%d_%d.tiff', strcat(filename,'_viable'), block_struct.location(1),block_struct.location(2)));
...
fid = fopen( '/raid/data/home/matlab/Mousumi/patch_dataset/MultiResolution/128_train.csv', 'a' );
% for jj = 1 : length( train_label_matrix )
%     fprintf( fid, '%s,%d\n', train_label_matrix(jj,1), train_label_matrix(jj,2));
% end

     for k=1:size(train_label_matrix,1)
         fprintf(fid,'%s,%d\n',train_label_matrix{k,:});
     end
