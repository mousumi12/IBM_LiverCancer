...
function [] = test_patch_generate_256(name1)
    ...
    if ((tissue_region / (512*512) > 0.25) && (count_bg / (512*512) <0.7) && (row == 512) && (col == 512))    %check the ratio of total no of pixels in [512 * 512] image , to remove the background patches.     
        ...
        output_filename1 = strcat('/raid/data/home/matlab/Mousumi/dataset/256/test/',sprintf('%s_%d_%d.tiff', filename, block_struct.location(1),block_struct.location(2)));
