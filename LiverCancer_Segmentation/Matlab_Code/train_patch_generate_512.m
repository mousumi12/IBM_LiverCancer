...
function [] = train_patch_generate_512(name1)
    ...
    if ((tissue_region / (1024*1024) > 0.25) && (count_bg / (1024*1024) <0.7) && (row == 1024) && (col == 1024))    %check the ratio of total no of pixels in [512 * 512] image , to remove the background patches.     
        
        %Resize the patch to get patch of size 1024x1024
        J =  block_struct.data;
        
        J1 = imresize(J,.5,'Antialiasing',true);       
        output_filename1 = strcat('/raid/data/home/matlab/Mousumi/dataset/512/train/',sprintf('%s_%d_%d.tiff', filename, block_struct.location(1),block_struct.location(2)));
