...
function [] = val_patch_with_tumor_256(name1)
...
%     global label_matrix
    ...
    if ((tissue_region / (512*512) > 0.25) && (count_bg /(512 * 512) < 0.7) && (row == 512) && (col == 512))    %check the ratio of total no of pixels in [512 * 512] image , to remove the background patches.     
        ...
        output_filename = strcat('/raid/data/home/matlab/Mousumi/patch_dataset/256_withGT/256/Val/',sprintf('%s_%d_%d.tiff', filename, block_struct.location(1),block_struct.location(2)));
        output_filename1 = strcat('/raid/data/home/matlab/Mousumi/patch_dataset/256_withGT/256_ViableTumor/Val/',sprintf('%s_%d_%d.tiff', strcat(filename,'_viable'), block_struct.location(1),block_struct.location(2)));
...
fid = fopen( '/raid/data/home/matlab/Mousumi/patch_dataset/256_withGT/256_val.csv', 'a' );
