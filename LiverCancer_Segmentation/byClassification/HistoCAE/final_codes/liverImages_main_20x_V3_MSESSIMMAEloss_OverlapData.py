...
# Load dataset
train_data = np.load('../datasets_Overlap/datasets_Overlap/datasets_20x_train/full_x.npy')
train_labels = np.load('../datasets_Overlap/datasets_Overlap/datasets_20x_train/full_y.npy')
val_data = np.load('../datasets_Overlap/datasets_Overlap/datasets_20x_val/full_x.npy')
val_labels = np.load('../datasets_Overlap/datasets_Overlap/datasets_20x_val/full_y.npy')
test_data = np.load('../datasets_Overlap/datasets_Overlap/datasets_20x_test/full_x.npy')
test_labels = np.load('../datasets_Overlap/datasets_Overlap/datasets_20x_test/full_y.npy')
...
cp_cb = ModelCheckpoint(filepath = 'autoencoder_20x_V3_MSESSIMMAEloss_OverlapData.h5', monitor='val_loss', verbose=1, save_best_only=True, mode='auto')
...
plt.legend()
fig.savefig('../Figures/Train_Val_Loss_reconst_20x_V3_MSESSIMMAEloss_OverlapData.png')
#plt.show()

#############################################################################

# Show the reconstruction result from autoencoder
autoencoder.load_weights('autoencoder_20x_V3_MSESSIMMAEloss_OverlapData.h5')

score = autoencoder.evaluate(test_data, test_data, verbose=1)
print(score)
f = open( 'file_recons_20x_V3_MSESSIMMAEloss_OverlapData.txt', 'a' )
...
def showOrigDec2(orig, dec, num=100):
    ...
    import scipy.misc
    n = num
    resultDir = "Test_result_20x_V3_MSESSIMMAEloss_OverlapData/"
...
def fc(enco):
    ...
    x = Conv2D(512, (3, 3), strides=(2, 2), activation='relu', padding='same')(x)
    x = BatchNormalization()(x)
    ...
    flat = Flatten()(x)
    den = Dense(256, activation='relu', kernel_regularizer=l2(0.01))(flat)
...
cp_cb = ModelCheckpoint(filepath = 'autoencoder_classification_20x_V3_MSESSIMMAEloss.h5', monitor='val_loss', verbose=1, save_best_only=True, mode='auto')
...
plt.legend()
fig.savefig('../Figures/Train_Val_accuracy_classification_20x_V3_MSESSIMMAEloss.png')
...
plt.legend()
#plt.show()
fig.savefig('../Figures/Train_Val_Loss_20x_V3_MSESSIMMAEloss.png')

######################################################

full_model.load_weights('autoencoder_classification_20x_V3_MSESSIMMAEloss.h5')

# Model Evaluation on the Test Set
test_eval = full_model.evaluate(test_data, test_Y_one_hot, verbose=0)
print('Test loss:', test_eval[0])
print('Test accuracy:', test_eval[1])
#f = open( 'file_classification_bottleneck_HistoVAE_V2_withoutflatten_10x_V3_200epoch_MSESSIMMAEloss_noDataAug_FCV1.txt', 'a' )
