...
def test_net(x, n):
    ...
    with tf.Session() as sess:
        with open(BASEDIR+"/dataset/" + RESNET_V2 +
                  "_submission_01_01_0113.csv", 'w') as sub:
            ...
            #print('Test acc:')
            #eval_.compute_accuracy(xp, BATCH_SIZE, is_train, x_val,y_val, prediction, sess)
...
def main():
    # get data
    x_test, x_name = load_data.load('test_resize_01_01_0113', label=False)
    global SIZE
    SIZE = np.size(x_name)
